...
BEGIN
  ...
  IF selected_product.category_id IS NOT NULL THEN
    SELECT category.*
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'product_description_generation_invalid_state';
    END IF;
  END IF;
  ...
  category_slug := CASE
    WHEN selected_product.category_id IS NULL THEN NULL
    ELSE selected_category.slug
  END;
  category_name := CASE
    WHEN selected_product.category_id IS NULL THEN NULL
...
BEGIN
  ...
  LOOP
    ...
    IF generated_description IS DISTINCT FROM
      ...
      OR char_length(generated_description) > 300
  ...
  IF p_title_proposal IS NOT NULL THEN
    ...
    IF normalized_title_proposal = ''
      ...
      OR char_length(normalized_title_proposal) > 50
    THEN
      RAISE EXCEPTION 'product_description_generation_output_invalid'
        USING ERRCODE = '22023';
    END IF;
